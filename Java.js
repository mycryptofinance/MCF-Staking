...
function getExplorerUrl(txHash) {
    const config = CONTRACT_CONFIG[currentChainId] || CONTRACT_CONFIG[11155111]; // По умолчанию Sepolia
...
async function switchNetwork(chainId) {
    if (chainId === 97) { // BNB Testnet
        const success = await addBNBTestnet();
        if (!success) {
            openModal('error', "Cannot switch to BNB Testnet. Please add it manually.");
            return;
        }
    }
    ...
    try { 
        ...
        // 4902 is the code for chain not found, which is handled by addBNBTestnet above
